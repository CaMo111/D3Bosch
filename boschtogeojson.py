...
def parse_line(line, prev_coordinates):
    ...
    # Check if the participant is 1
    if fields[1] == '1':
        timestamp_str = fields[0]
        timestamp = datetime.strptime(timestamp_str, "%Y-%m-%d %H:%M:%S")
        
        # Filter for journeys before 12 PM on "2023-01-18"
        if timestamp.date() == datetime(2023, 1, 18).date() and timestamp.time() < datetime.strptime("12:00:00", "%H:%M:%S").time():
            colour = 'red'
